...
(fn common-groups []
  `(do
     ...
     ;; Error messaging (Error itself differs per theme)
     ...
     ;; Diffs (DiffAdd, DiffChange, DiffDelete differ per theme)
     (ln DiffText Visual)
     ...
     ;; Todo comments (TodoBgTODO/TodoFgTODO are lightearth-only)
     (hl Todo :fg (bg) :bg (fg))
     (ln "@text.todo" Todo)
     ;; Language overrides
     ...
     ;; Fzf-Lua (FzfLuaCursor and FzfLuaBackdrop differ per theme)
